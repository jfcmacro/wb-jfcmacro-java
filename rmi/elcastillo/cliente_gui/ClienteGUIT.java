...
public class ClienteGUIT {

    JTextField registroTextField, selladorTextField;
    
    public ClienteGUIT(Registro rServer, Sellador sServer) {
	...
	registroButton.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent event) {
		    new Thread(new HTRegistro(r, registroTextField)).start();
		}
	    });

	selladorButton.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent event) {
		    new Thread(new HTSellador(s, selladorTextField)).start();
		}
	    });
	...
	if (args.length != 2) {
	    System.err.println("Usage: ClienteGUIT <rmiregistro> <rmisellador>");
	    System.exit(1);
	}
